...
关于这三个方法的原理其实就非常简单了，简单来说就是把当前语言以及当前语言包的配置挂在 Vue 的原型属性上，并将这两个属性使用 Vue utils 中的 `defineReactive` 方法变成响应式对象。然后这三个方法的核心就是针对原型上的这两个响应式变量进行相应的读写操作了。在 `add` 方法中的 `deepAssign` 方法倒是值得再细聊一下。可以单开一篇文章再剖析一下这个方法。  
